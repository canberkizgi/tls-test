...
public class TLSTest {
    ...
    public void httpRequest_Returns200OK() throws Exception {
        ...
        try {
            HttpResponse response = httpclient.execute(new HttpGet("http://" + baseUrl + "/echo/this"));
    ...
    @Test
    public void httpRequest_WithNoSSLContext_ThrowsSSLException() throws Exception {
        HttpServer server = createLocalTestServer(NO_SSL_CONTEXT, ONE_WAY_SSL);
        server.start();
    ...
    public void httpsRequest_WithSSLAndValidatingCertsButNoClientTrustSture_ThrowsSSLException() throws Exception {
        ...
        HttpServer server = createLocalTestServer(serverSSLContext, ONE_WAY_SSL);
        server.start();
    ...
    public void httpsRequest_WithSSLAndTrustingAllCertsButNoClientTrustSture_Returns200OK() throws Exception {
        ...
        try {
            HttpResponse response = httpclient.execute(new HttpGet("https://" + baseUrl + "/echo/this"));
            assertThat(response.getStatusLine().getStatusCode(), equalTo(200));
        } finally {
            server.stop();
        }
